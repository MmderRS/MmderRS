...
// 成语列表
const chengyuList = [
    '生日快乐', '岁岁平安', '福暖四季', '万事顺意', '喜乐无忧',
    '前程似锦', '平安喜乐', '岁岁欢愉', '心想事成', '顺遂无忧'
];
let chengyuIndex = 0;

function spawnChengyu() {
    const el = document.createElement('div');
    el.className = 'chengyu-float';
    el.textContent = chengyuList[chengyuIndex % chengyuList.length];
    chengyuIndex++;
    // 随机水平位置，避免贴边
    const maxLeft = window.innerWidth - 160;
    el.style.left = Math.max(20, Math.floor(Math.random() * maxLeft)) + 'px';
